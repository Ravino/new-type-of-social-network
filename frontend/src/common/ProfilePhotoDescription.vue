...
<!--                         :aria-labelledby="`postSettings` + getTimePost">-->
                    <div class="dropdown-menu dropdown-menu-right py-3 px-0" :aria-labelledby="`postSettings`">
...
<script>
 ...
 export default {
  ...
  computed: {
   ...
   userAvatar() {
       if (this.$root.$auth.user.avatar === null) {
           return this.noAvatar;
       }

       return this.$root.$auth.user.avatar.image.thumb.path;
   },
   userName() {
    return this.$root.$auth.user.firstName;
   },
   userSurname() {
    return this.$root.$auth.user.lastName;
   },
   // getTimePost() {
  ...
  methods: {
    ...
    //
    //     try {
    //         response = await this.$root.$api.$image.likePostImage(this.image.id);
    //     } catch (e) {
